# HistogramGlyph Class

The `histogram_glyph` module provides the `HistogramGlyph` class for creating
...
::: cleopatra.glyphs.stats.histogram_glyph.HistogramGlyph
    options:
...
![One Histogram Example](../images/histogram_glyph/one-histogram.png)
...
![Three Histogram Example](../images/histogram_glyph/three-histogram.png)
...
fig, ax, artists = HistogramGlyph(data).boxplot(labels=["a", "b", "c"], notch=True)
```
...
fig, ax, artists = HistogramGlyph(data).multiboxplot(
...
fig, ax, bars = HistogramGlyph(yearly).stripes(cmap="RdBu_r")
```
