...
_GROUPED_KWARG_HINTS: dict[str, str] = {
    ...
    "hatches": "contour=Contour(hatches=[...], fill=False)",
    "hatch_color": "contour=Contour(hatch_color=...)",
    ...
    # loose `alpha=` locally instead (see its `_reject_loose_alpha`). `fill` is
    # kept out for the same reason -- it is a generic name a caller of another
    # glyph (e.g. filled polygons) might pass -- and `ArrayGlyph` rejects a
    # loose `fill=` locally (see its `_reject_loose_fill`).
}
...
def _group_option_items(group: Any) -> dict:
    ...
    Raises:
        TypeError: If `group` is neither a `matplotlib.colors.Normalize` nor a
            grouped parameter object (one exposing `to_options()`) -- e.g. a
            loose colour string passed where a `ColorScaling` is expected.
    """
    if isinstance(group, colors.Normalize):
        return {"norm": group}
    if not hasattr(group, "to_options"):
        raise TypeError(
            f"expected a grouped parameter object -- one exposing to_options(), "
            f"such as ColorScaling, Contour, Classify, CellValues or DataStyle "
            f"(or a matplotlib Normalize) -- got {type(group).__name__} "
            f"{group!r}. These parameters take a typed style object, not the "
            f"loose value the option they replaced accepted."
        )
