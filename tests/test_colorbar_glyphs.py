...
def _hexbin():
    """A HexbinGlyph over a random point cloud."""
    return HexbinGlyph(_RNG.random(80), _RNG.random(80), gridsize=8)
...
GLYPHS = {
    ...
    "hexbin": (
        ...
        lambda g, cb: g.plot(colorbar=cb),
        lambda g: g.cbar,
        lambda: HexbinGlyph(_RNG.random(80), _RNG.random(80), cbar_label="X"),
    ),
    "mesh": (
...
class TestColorBarMethods:
    ...
    def test_resolve_false_suppresses(self):
        ...
        """
        assert ColorBar.resolve(False) == {"add_colorbar": False}, (
    ...
    def test_resolve_instance_delegates_to_to_options(self):
        ...
        assert ColorBar.resolve(cb) == cb.to_options(), (
            "instance should delegate to to_options"
        )
    ...
    def test_specifies_placement(self, kwargs, expected):
        ...
        assert got is expected, (
            f"specifies_placement({kwargs}) -> {got}, expected {expected}"
        )
